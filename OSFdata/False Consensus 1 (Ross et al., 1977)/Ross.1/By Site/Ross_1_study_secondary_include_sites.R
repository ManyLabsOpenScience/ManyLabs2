...
##
## False Consensus 1 (Ross et al., 1977): Ross_1_study_secondary_include_sites
##
...
ML2.df <- ML2.df  %>% dplyr::select(1,6,23,29,520,521,522,523,524,525,526,527,528,529,530,531,534,535,536) %>% dplyr::filter(is.character(source))
...
# To see the function code type:varfun.Ross.1, or lookup in manylabRs_SOURCE.R
...
stat.test   <- try.CATCH(with(ML2.var[[g]],t.test(Peers ~ You, conf.level=stat.params$conf.level, var.equal = stat.params$var.equal, alternative = stat.params$alternative)))
